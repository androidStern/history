...
export default function WorldView() {
  ...
  const { cameraX, activeSegments, currentScene, goForward, goBack, sceneHistory } =
    useSceneTransitions({
  ...
  return (
    ...
    >
      {activeSegments.map(segment => (
        <ParallaxLayers
          key={`${segment.id}-${segment.uniqueVisitId}`}
          segment={segment}
          cameraX={cameraX}
        />
      ))}
      ...
      {currentScene && (
        <DialogueOverlay
          ...
          onChoiceSelect={goForward}
          goBack={goBack}
          showBack={sceneHistory.length > 1}
        />
      )}
    </div>
  )
