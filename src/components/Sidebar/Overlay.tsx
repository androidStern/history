...
export default function Overlay({ draggedItem }: { draggedItem: DraggedItemType | null }) {
  ...
  return (
    ...
    >
      {draggedItem?.type === 'dialogue' ? (
        ...
        >
          <DialogueEditor initialText={scenes[draggedItem.sceneId]?.dialogue.find(d => d.id === draggedItem.data.id)?.text || ''} dialogueId={draggedItem.data.id} sceneId={draggedItem.sceneId} />
        </div>
      ) : draggedItem ? (
        <SidebarGroup>
          <SidebarMenu>
